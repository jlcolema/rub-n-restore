<?php 
    if (!defined('ABSPATH'))
		...
		//LIVE SITE
		//Edited 07/19/2023 - AJ Designs - add generic color, prices and individual add to cart.
...
//Output the Product image
    ...
    function rnr_get_product_image($GFproductID){    
          ...
          return $product->get_image('woocommerce_gallery_thumbnail'); //Here is the product image  
...
//no quantity no size    
        if( rgar( $entry, '285' ) ){//must have the product No size on simple products
            ...
            $productCM0IMG        =  $product->get_image('woocommerce_gallery_thumbnail');
            $productCM0QTY        =   rgar( $entry, '287' ); //the gF quantity field
			
			if($productCM0QTY  ==''){$productCM0QTY       =  "1"  ;}
			
            $productCM0VARid      =  $prodID ;
			
			$variableCM0Product      =  wc_get_product( $productCM0VARid ); // new as of 2023/06 - for showing price in eval
			
			if($variableCM0Product !="0"){
				
			$prodCM0Price		    =  $variableCM0Product->get_price();
			
			}
			
			else {
				
				$prodCM0Price	="Choose options";
...
//NEW generic color product - this will likely replace all color options. 6-27-2023: AJ Designs
		      
if( rgar( $entry, '359' ) ){//check if generic color is checked
            
			$ACFgenericColor = rgar( $entry, '363' );
			
			$product00IMG   = '<img height="100" width="100" src=" '. get_site_url() . '/wp-content/uploads/2022/03/bottle-colors-wreath-bright-150x150.jpg" alt="Leather & Vinyl Colors Wheel">'; //generic image
			
			$GFsize             = array('pa_size' => rgar( $entry, '360' ),); //To feed to the var id function
			$product00SIZE       =  rgar( $entry, '360' );  
			$product00QTY        =  rgar( $entry, '361' ); //the gF quantity field
			
			if($product00QTY =='') //in case it is not set
         	   {$product00QTY       =  "1"  ;}
             //new
			// We need to get a price based on the quantity.
			// To ensure future price compatibility we will use the price from marine white, ID: 44773

			$product00VARid      =  rnr_get_variation_ID('44773', $GFsize); 
			$variableProduct   =  wc_get_product($product00VARid); // new as of 2023/06 - for showing price in eval
			$basePrice		    =  $variableProduct->get_price();
			$prod00Price = $basePrice * $product00QTY; //this gets actual price
			$prod00Price = 'TBD'; //override
			//new
			
			$product00addtoCART  = '<a href="' . get_site_url() . '/product-category/leather-vinyl-dyes/"> Chose your color, size & quantity in the shop </a>';
		...
		else{$product00addtoCART =  '' ; $ACFgenericColor ='';    }    
...
//Product 1 - Color 1
        ...
        if( rgar( $entry, '253' ) && rgar( $entry, '257' )){//must have the product and size
			...
			///////////NEW////////////
			$variable1Product   =  wc_get_product($product1VARid); // new as of 2023/06 - for showing price in eval
			if($variable1Product !="0"){
			$prod1Price		    =  $variable1Product->get_price();
			}
			else {$prod1Price	="Choose options";}			
			///////////NEW////////////
...
//Product 2 - Color 2
        ...
        if( rgar( $entry, '262' ) && rgar( $entry, '261' )){//must have the product and size
			...
			///////////NEW////////////
			$variable2Product   =  wc_get_product($product2VARid); // new as of 2023/06 - for showing price in eval
			if($variable4_12Product !="0"){
			$prod2Price		    =  $variable2Product->get_price();
			}
			else {$prod2Price	="Choose options";}			
			///////////NEW////////////  
...
//Product 3 - Color 3
        ...
        if( rgar( $entry, '265' ) && rgar( $entry, '264' )){//must have the product and size
			...
			///////////NEW////////////
			$variable3Product   =  wc_get_product($product3VARid); // new as of 2023/06 - for showing price in eval
			if($variable3Product !="0"){
			$prod3Price		    =  $variable3Product->get_price();
			}
			else {$prod3Price	="Choose options";}			
			///////////NEW////////////  
...
//Product 4 - Color 4
        ...
        if( rgar( $entry, '272' ) && rgar( $entry, '271' )){//must have the product and size
			...
			///////////NEW////////////
			$variable4Product   =  wc_get_product($product4VARid); // new as of 2023/06 - for showing price in eval
			if($variable4Product !="0"){
			$prod4Price		    =  $variable4Product->get_price();
			}
			else {$prod4Price	="Choose options";}			
			///////////NEW////////////  
...
//Product 4.1 Color 5
        ...
        if( rgar( $entry, '303' ) && rgar( $entry, '304' )){//must have the product and size
			...
			///////////NEW////////////
			$variable4_1Product   =  wc_get_product($product4_1VARid); // new as of 2023/06 - for showing price in eval
			if($variable4_1Product !="0"){
			$prod4_1Price		    =  $variable4_1Product->get_price();
			}
			else {$prod4_1Price	="Choose options";}
			///////////NEW////////////  
...
//Product 4.2 - Satin & Dull Sealer
        ...
        if( rgar( $entry, '344' ) && rgar( $entry, '346' )){//must have the product and size
			...
			///////////NEW////////////
			$variable4_12Product   =  wc_get_product($product4_12VARid); // new as of 2023/06 - for showing price in eval
			if($variable4_12Product !="0"){
			$prod4_12Price		    =  $variable4_12Product->get_price();
			}
			else {$prod4_12Price	="Choose options";}
			///////////NEW////////////  
...
//Product 5 - Filler or Filler Kit 1
        ...
        if( rgar( $entry, '275' ) && rgar( $entry, '276' ) ){//must have the product & size
			...
			// must have the tool & sandpaper set
			///////////NEW////////////
			$variable5Product   =  wc_get_product($product5VARid); // new as of 2023/06 - for showing price in eval
			if($variable5Product !="0"){
			$prod5Price		    =  $variable5Product->get_price();
			}
			else {$prod5Price	="Choose options";}
			///////////NEW////////////  
...
//Product 5.01 - Filler or Filler Kit 2
        ...
        if( rgar( $entry, '300' ) && rgar( $entry, '301' )){//must have the product and size
			...
			$variable5_01Product   =  wc_get_product($product5_01VARid);  
			
			if($variable5_01Product !="0"){
			$prod5_01Price		    =  $variable5_01Product->get_price();
			}
			else {$prod5_01Price	="Choose options";}
...
// Subpatch,  Subpatch KIT, 3m Glue 1       
       if( rgar( $entry, '279' ) ){//must have the product. No size on simple products
			...
			///////////NEW////////////
			$variable6Product   =  wc_get_product($product6VARid); // new as of 2023/06 - for showing price in eval
			if($variable6Product !="0"){
			$prod6Price		    =  $variable6Product->get_price();
			}
			else {$prod6Price	="Choose options";}
			///////////NEW////////////  
...
// Subpatch,  Subpatch KIT, 3m Glue 2       
       if( rgar( $entry, '312' ) ){//must have the product. No size on simple products
			...
			///////////NEW////////////
			$variable6_1Product   =  wc_get_product($product6_1VARid); // new as of 2023/06 - for showing price in eval
			if($variable6_1Product !="0"){
			$prod6_1Price		    =  $variable6_1Product->get_price();
			}
			else {$prod6_1Price	="Choose options";}
			///////////NEW////////////  
...
//Product 6.02 - TOP Patch
        
        if( rgar( $entry, '368' ) && rgar( $entry, '367' )&& rgar( $entry, '366' )){//must have the product, material, size
        
            $GFproductID = rgar( $entry, '368' );
			
            $GFattributes = array( //The attributes - we need this to feed the rnr_match_prod_var() function.
                'attribute_pa_size' => rgar( $entry, '366' ),
				'attribute_pa_material' => rgar( $entry, '367' ),
				 
                );    
	
            $product6_02VARid      =  rnr_get_variation_ID_kit($GFproductID, $GFattributes);  // access the function with kit attributes   
			
			///////////NEW////////////
			
			$variable6_02Product   =  wc_get_product($product6_02VARid);  
			
			if($variable6_02Product !="0"){
			$prod6_02Price		    =  $variable6_02Product->get_price();
			}
			else {$prod6_02Price	="Choose options";}

			///////////NEW////////////  
			
			$product6_02NAME       =  rnr_get_product_name($GFproductID);
            $product6_02SIZE      =  rgar( $entry, '366' );    
			
			$product6_02material       =  rgar( $entry, '367' ); // new as of 07/25/2023
			
			/*$tools6_02='';
			if ($product6_02tools == 'needle-card')
				{$tools6_02='Needle Tool & Spreader Card';}
			if ($product6_02tools == 'knife-card')
				{$tools6_02='Palette Knife & Spreader Card';}
			if ($product6_02tools == 'needle-knife-card')
				{$tools6_02='Needle Tool, Palette Knife & Spreader Card';}*/
								
			
           // $product6_02QTY        =  rgar( $entry, '314' ); //the gF quantity field
            //if($product6_02QTY =='')
           // {$product6_02QTY       =  "1"  ;}
			$product6_02QTY       =  "1"  ;
            $product6_02IMG        =  rnr_get_product_image($GFproductID);
            $product6_02addtoCART  =  $product6_02QTY . 'x' . $product6_02VARid . ',' ;
            $product6_02           =  'true';
            $hasPRODUCT            =  'true';
            
        }else{$product6_02addtoCART =  '' ;}
...
//we need to set the GFsize to grit for sandpaper        
        if( rgar( $entry, '282' ) ){//must have the product No size on simple products
			...
			$variable7Product   =  wc_get_product($product7VARid); // new as of 2023/06 - for showing price in eval
			if($variable7Product !="0"){
			$prod7Price		    =  $variable7Product->get_price();
			}
			else {$prod7Price	="Choose options";}			
...
//we need to set the GFsize to grit for sandpaper        
        if( rgar( $entry, '288' ) ){//must have the product No size on simple products
			...
			///////////NEW////////////
			$variable8Product   =  wc_get_product($product8VARid); // new as of 2023/06 - for showing price in eval
			if($variable8Product !="0"){
			$prod8Price		    =  $variable8Product->get_price();
			}
			else {$prod8Price	="Choose options";}					
			///////////NEW////////////  
...
//Size must be set programatically    
        ...
        if( rgar( $entry, '291' ) ){//must have the product No size on simple products
			...
			///////////NEW////////////
			$variable9Product   =  wc_get_product($product9VARid); // new as of 2023/06 - for showing price in eval
			if($variable9Product !="0"){
			$prod9Price		    =  $variable9Product->get_price();
			}
			else {$prod9Price	="Choose options";}					
			///////////NEW////////////  
...
//Size must be set programatically    
        ...
        if( rgar( $entry, '294' ) ){//must have the product No size on simple products
			...
			///////////NEW////////////
			$variable10Product   =  wc_get_product($product10VARid); // new as of 2023/06 - for showing price in eval
			if($variable10Product !="0"){
			$prod10Price		    =  $variable10Product->get_price();
			}
			else {$prod10Price	="Choose options";}					
			///////////NEW////////////  
...
//Size must be set programatically    
        ...
        if( rgar( $entry, '297' ) ){//must have the product No size on simple products
			...
			///////////NEW////////////
			$variable11Product   =  wc_get_product($product11VARid); // new as of 2023/06 - for showing price in eval
			if($variable11Product !="0"){
			$prod11Price		    =  $variable11Product->get_price();
			}
			else {$prod11Price	="Choose options";}					
			///////////NEW////////////  
...
//Size must be set programatically    
        ...
        if( rgar( $entry, '317' ) ){//must have the product No size on simple products
			...
			///////////NEW////////////
			$variable12Product   =  wc_get_product($product12VARid); // new as of 2023/06 - for showing price in eval
			if($variable12Product !="0"){
			$prod12Price		    =  $variable12Product->get_price();
			}
			else {$prod12Price	="Choose options";}		
...
/*            elseif( $product->is_type( 'variable' ) &&  rgar( $entry, '347' ) ){
			...
			///////////NEW////////////
			$variable13Product   =  wc_get_product($product13VARid); // new as of 2023/06 - for showing price in eval
			if($variable13Product !="0"){
			$prod13Price		    =  $variable13Product->get_price();
			}
			else {$prod13Price	="Choose options";}		
...
/*            elseif( $product->is_type( 'variable' ) &&  rgar( $entry, '347' ) ){
			...
			///////////NEW////////////
			$variable14Product   =  wc_get_product($product14VARid); // new as of 2023/06 - for showing price in eval
			if($variable14Product !="0"){
			$prod14Price		    =  $variable14Product->get_price();
			}
			else {$prod14Price	="Choose options";}					
			///////////NEW////////////  
...
$notification['message'] .= ' 
        ...
        <table width="600" border="0" cellspacing="0" cellpadding="0" style="background-color:#f7f3ed;border:0; padding:20px;">
        <tr>
        <td> <!--containter table -->    
        
        
        <table width="600" border="1" cellspacing="0" cellpadding="12" style="background-color:#225b66;color:#ffffff;border-bottom:0;font-weight:bold;line-height:100%;vertical-align:middle;text-align:center">
          <tr>
            <td style="background-color:#225b66;color:#ffffff;border-bottom:0;font-weight:bold;line-height:100%;vertical-align:middle;border:0;"><a href="https://rubnrestore.com" title="Rub n Restore"><img src="https://rubnrestore.com/wp-content/uploads/rubnrestore-logo-long-white-1.png" alt="Rub \'n Restore Logo" width="300" height="41" /> </td>
          </tr>
          <tr>
            <td style="background-color:#225b66;color:#ffffff;border:0;font-weight:bold;line-height:100%;vertical-align:middle;" ><h1 style="font-size:28px;font-family:Marcellus, Roboto, Lucida Sans Unicode, sans-serif; line-height:150%;margin:0;text-align:center;color:#ffffff">How To Do It & Product Suggestions</h1></td>
          </tr>
 ...
 if ($hasPRODUCT=='true'){//header
     ...
     $notification['message'] .= ' 
      <tr>
        ...
        <th scope="col" style="padding:12px;border: 1px solid #e2ddd3;">QTY</th>
        <th scope="col" style="padding:12px;border: 1px solid #e2ddd3;">Size</th>
		<th scope="col" style="padding:12px;border: 1px solid #e2ddd3;">Price</th>
		<th scope="col" style="padding:12px;border: 1px solid #e2ddd3;">Purchase</th>
...
//product CM0
 ...
 if ($productCM0=='true'){
 	...
 	$addToCart = '<a target="_blank" rel="noopener" href="https://rubnrestore.com/cart/?wpo_fill_cart=' .  $productCM0addtoCART . '" > ' . 	    $productCM0IMG . ' <span style="    padding: 5px;    background: #016ab5;    text-align: center;    color: #fff;        font-size: 14px;    display: block;    border-radius: 4px;">Add to Cart </span> </a>';
    ...
    $notification['message'] .= '   
      <tr>
        ...
        <td style="padding:12px;border: 1px solid #e2ddd3;">'                        . '$' . $prodCM0Price . '</td>
		<td style="padding:12px;border: 1px solid #e2ddd3; text-align:center;">'     .  $addToCart . '</td>
      </tr>';
     } 
 ...
 if ($product00  =='true'){ 
		...
		$addToCart = '<a target="_blank" rel="noopener" href="'  . get_site_url() . '/product-category/leather-vinyl-dyes/" > ' . $product00IMG . ' <span style="    padding: 5px;    background: #016ab5;    text-align: center;    color: #fff;        font-size: 14px;    display: block;    border-radius: 4px;">Choose Color</span>
					</a>';
	 $notification['message'] .= '   
	 
      <tr>
	    
        <td style="padding:12px;border: 1px solid #e2ddd3;">'                        . $ACFgenericColor .'</td>
        <td style="padding:12px;border: 1px solid #e2ddd3;">'                        . $product00QTY  . '</td>
        <td style="padding:12px;border: 1px solid #e2ddd3;">'                        . $product00SIZE . '</td>
		<td style="padding:12px;border: 1px solid #e2ddd3;">'                        . '$' . $prod00Price . '</td>
		<td style="padding:12px;border: 1px solid #e2ddd3; text-align:center;">'     .  $addToCart . '</td>
...
//product 1
 if ($product1=='true'){
	  ...
	  $addToCart = '<a target="_blank" rel="noopener" href="https://rubnrestore.com/cart/?wpo_fill_cart=' .  $product1addtoCART . '" > ' . $product1IMG . ' <span style="    padding: 5px;    background: #016ab5;    text-align: center;    color: #fff;        font-size: 14px;    display: block;    border-radius: 4px;">Add to Cart </span>
                </a>';
	...
	<td style="padding:12px;border: 1px solid #e2ddd3;">'                        . '$' . $prod1Price . '</td>	
		<td style="padding:12px;border: 1px solid #e2ddd3; text-align:center;">'     .  $addToCart . '</td>
...
//product 2
 if ($product2=='true'){
	 ...
	 $addToCart = '<a target="_blank" rel="noopener" href="https://rubnrestore.com/cart/?wpo_fill_cart=' .  $product2addtoCART . '" > ' . $product2IMG . ' <span style="    padding: 5px;    background: #016ab5;    text-align: center;    color: #fff;        font-size: 14px;    display: block;    border-radius: 4px;">Add to Cart </span>
                </a>';
      ...
      <tr>
	    ...
	    <td style="padding:12px;border: 1px solid #e2ddd3;">'                        . '$' . $prod2Price . '</td>		
        <td style="padding:12px;border: 1px solid #e2ddd3; text-align:center;">'   . $addToCart  . '</td>
      </tr>';  
     }
//product 3     
...
$addToCart = '<a target="_blank" rel="noopener" href="https://rubnrestore.com/cart/?wpo_fill_cart=' .  $product3addtoCART . '" > ' . $product3IMG . ' <span style="    padding: 5px;    background: #016ab5;    text-align: center;    color: #fff;        font-size: 14px;    display: block;    border-radius: 4px;">Add to Cart </span>
                </a>'; 
        $notification['message'] .= '   
      <tr>
	    ...
	    <td style="padding:12px;border: 1px solid #e2ddd3;">'                        . '$' . $prod3Price . '</td>        
		<td style="padding:12px;border: 1px solid #e2ddd3; text-align:center;">'   . $addToCart  . '</td>
      </tr>';
     } 
//product 4
 if ($product4=='true'){
	 ...
	 $addToCart = '<a target="_blank" rel="noopener" href="https://rubnrestore.com/cart/?wpo_fill_cart=' .  $product4addtoCART . '" > ' . $product4IMG . ' <span style="    padding: 5px;    background: #016ab5;    text-align: center;    color: #fff;        font-size: 14px;    display: block;    border-radius: 4px;">Add to Cart </span>
                </a>';
        $notification['message'] .= '   
      <tr>
	    ...
	    <td style="padding:12px;border: 1px solid #e2ddd3;">'                        . '$' . $prod4Price . '</td>        
		<td style="padding:12px;border: 1px solid #e2ddd3; text-align:center;">'   . $addToCart  . '</td>
      </tr>';
     } 
//product 4.1
 if ($product4_1=='true'){
	 ...
	 $addToCart = '<a target="_blank" rel="noopener" href="https://rubnrestore.com/cart/?wpo_fill_cart=' .  $product4_1addtoCART . '" > ' . $product4_1IMG . ' <span style="    padding: 5px;    background: #016ab5;    text-align: center;    color: #fff;        font-size: 14px;    display: block;    border-radius: 4px;">Add to Cart </span>
                </a>';
        $notification['message'] .= '   
      <tr>
	    ...
	    <td style="padding:12px;border: 1px solid #e2ddd3;">'                        . '$' . $prod4_1Price . '</td>        
	    <td style="padding:12px;border: 1px solid #e2ddd3; text-align:center;">'   . $addToCart . '</td>
      </tr>';
     }  
...
//product 4.12
 if ($product4_12=='true'){
	 ...
	 $addToCart = '<a target="_blank" rel="noopener" href="https://rubnrestore.com/cart/?wpo_fill_cart=' .  $product4_12addtoCART . '" > ' . $product4_12IMG . ' <span style="    padding: 5px;    background: #016ab5;    text-align: center;    color: #fff;        font-size: 14px;    display: block;    border-radius: 4px;">Add to Cart </span>
                </a>';
        $notification['message'] .= '   
      <tr>
	    ...
	    <td style="padding:12px;border: 1px solid #e2ddd3;">'                        . '$' . $prod4_12Price . '</td>        
		<td style="padding:12px;border: 1px solid #e2ddd3; text-align:center;">'   . $addToCart . '</td>
      </tr>';
     }  	             
//product 5      
 if ($product5=='true'){
	 	 ...
	 	 $addToCart = '<a target="_blank" rel="noopener" href="https://rubnrestore.com/cart/?wpo_fill_cart=' .  $product5addtoCART . '" > ' . $product5IMG . ' <span style="    padding: 5px;    background: #016ab5;    text-align: center;    color: #fff;        font-size: 14px;    display: block;    border-radius: 4px;">Add to Cart </span>
                </a>';
        $notification['message'] .= '   
      <tr>
	    ...
	    <td style="padding:12px;border: 1px solid #e2ddd3;">'                        . '$' . $prod5Price . '</td>        
		<td style="padding:12px;border: 1px solid #e2ddd3; text-align:center;">'   . $addToCart . '</td>
      </tr>';
     }
...
//product 5 .01     
 if ($product5_01=='true'){
	 	 $addToCart = '<a target="_blank" rel="noopener" href="https://rubnrestore.com/cart/?wpo_fill_cart=' .  $product5_01addtoCART . '" > ' . $product5_01IMG . ' <span style="    padding: 5px;    background: #016ab5;    text-align: center;    color: #fff;        font-size: 14px;    display: block;    border-radius: 4px;">Add to Cart </span>
                </a>';
        $notification['message'] .= '   
      <tr>
	    ...
	    <td style="padding:12px;border: 1px solid #e2ddd3;">'                        . '$' . $prod5_01Price . '</td>        
		<td style="padding:12px;border: 1px solid #e2ddd3; text-align:center;">'   . $addToCart . '</td>
      </tr>';
     }  
...
//product 6      
 if ($product6=='true'){
	 	 ...
	 	 $addToCart = '<a target="_blank" rel="noopener" href="https://rubnrestore.com/cart/?wpo_fill_cart=' .  $product6addtoCART . '" > ' . $product6IMG . ' <span style="    padding: 5px;    background: #016ab5;    text-align: center;    color: #fff;        font-size: 14px;    display: block;    border-radius: 4px;">Add to Cart </span>
                </a>';
        $notification['message'] .= '   
      <tr>
	    ...
	    <td style="padding:12px;border: 1px solid #e2ddd3;">'                        . '$' . $prod6Price . '</td>        
		<td style="padding:12px;border: 1px solid #e2ddd3; text-align:center;">'     . $addToCart  . '</td>
      </tr>';
     } 
//product 6_1      
 if ($product6_1=='true'){
	 	 ...
	 	 $addToCart = '<a target="_blank" rel="noopener" href="https://rubnrestore.com/cart/?wpo_fill_cart=' .  $product6_1addtoCART . '" > ' . $product6_1IMG . ' <span style="    padding: 5px;    background: #016ab5;    text-align: center;    color: #fff;        font-size: 14px;    display: block;    border-radius: 4px;">Add to Cart </span>
                </a>';
        $notification['message'] .= '   
      <tr>
	    ...
	    <td style="padding:12px;border: 1px solid #e2ddd3;">'                        . '$' . $prod6_1Price . '</td>        
	    <td style="padding:12px;border: 1px solid #e2ddd3; text-align:center;">'   . $addToCart . '</td>
      </tr>';
     }	
//product 6_02     
 if ($product6_02=='true'){
	 	 $addToCart = '<a target="_blank" rel="noopener" href="https://rubnrestore.com/cart/?wpo_fill_cart=' .  $product6_02addtoCART . '" > ' . $product6_02IMG . ' <span style="    padding: 5px;    background: #016ab5;    text-align: center;    color: #fff;        font-size: 14px;    display: block;    border-radius: 4px;">Add to Cart </span>
                </a>';
        $notification['message'] .= '   
      <tr>
        <td style="padding:12px;border: 1px solid #e2ddd3;">'                        . $product6_02NAME . ' </td>
        <td style="padding:12px;border: 1px solid #e2ddd3;">'                        . $product6_02QTY  . '</td>
        <td style="padding:12px;border: 1px solid #e2ddd3;">'                        . $product6_02SIZE . '</td>
	    <td style="padding:12px;border: 1px solid #e2ddd3;">'                        . '$' . $prod6_02Price . '</td>        
		<td style="padding:12px;border: 1px solid #e2ddd3; text-align:center;">'   . $addToCart . '</td>
      </tr>';
     }  	       	  
//product 7         
 if ($product7=='true'){
	 	 ...
	 	 $addToCart = '<a target="_blank" rel="noopener" href="https://rubnrestore.com/cart/?wpo_fill_cart=' .  $product7addtoCART . '" > ' . $product7IMG . ' <span style="    padding: 5px;    background: #016ab5;    text-align: center;    color: #fff;        font-size: 14px;    display: block;    border-radius: 4px;">Add to Cart </span>
                </a>';
        $notification['message'] .= '   
      <tr>
	    ...
	    <td style="padding:12px;border: 1px solid #e2ddd3;">'                        . '$' . $prod7Price . '</td>        
		<td style="padding:12px;border: 1px solid #e2ddd3; text-align:center;">'   . $addToCart  . '</td>
      </tr>';
     }     
//product 8     
 if ($product8=='true'){
	 	 	 ...
	 	 	 $addToCart = '<a target="_blank" rel="noopener" href="https://rubnrestore.com/cart/?wpo_fill_cart=' .  $product8addtoCART . '" > ' . $product8IMG . ' <span style="    padding: 5px;    background: #016ab5;    text-align: center;    color: #fff;        font-size: 14px;    display: block;    border-radius: 4px;">Add to Cart </span>
                </a>';
        $notification['message'] .= '   
      <tr>
	    ...
	    <td style="padding:12px;border: 1px solid #e2ddd3;">'                        . '$' . $prod8Price . '</td>        
	    <td style="padding:12px;border: 1px solid #e2ddd3; text-align:center;">'   . $addToCart  . '</td>
      </tr>';
     }          
//product 9         
 if ($product9=='true'){
	 	 	 ...
	 	 	 $addToCart = '<a target="_blank" rel="noopener" href="https://rubnrestore.com/cart/?wpo_fill_cart=' .  $product9addtoCART . '" > ' . $product9IMG . ' <span style="    padding: 5px;    background: #016ab5;    text-align: center;    color: #fff;        font-size: 14px;    display: block;    border-radius: 4px;">Add to Cart </span>
                </a>';
        $notification['message'] .= '   
      <tr>
	    ...
	    <td style="padding:12px;border: 1px solid #e2ddd3;">'                        . '$' . $prod9Price . '</td>        
		<td style="padding:12px;border: 1px solid #e2ddd3; text-align:center;">'     . $addToCart . '</td>
      </tr>';
     }          
//product 10     
 if ($product10=='true'){
	 	 	 ...
	 	 	 $addToCart = '<a target="_blank" rel="noopener" href="https://rubnrestore.com/cart/?wpo_fill_cart=' .  $product10addtoCART . '" > ' . $product10IMG . ' <span style="    padding: 5px;    background: #016ab5;    text-align: center;    color: #fff;        font-size: 14px;    display: block;    border-radius: 4px;">Add to Cart </span>
                </a>';
        $notification['message'] .= '   
      <tr>
	    ...
	    <td style="padding:12px;border: 1px solid #e2ddd3;">'                        . '$' . $prod10Price . '</td>        
		<td style="padding:12px;border: 1px solid #e2ddd3; text-align:center;">'     . $addToCart . '</td>
      </tr>';
     }          
...
//product 11    
 if ($product11=='true'){
	 	 	 ...
	 	 	 $addToCart = '<a target="_blank" rel="noopener" href="https://rubnrestore.com/cart/?wpo_fill_cart=' .  $product11addtoCART . '" > ' . $product11IMG . ' <span style="    padding: 5px;    background: #016ab5;    text-align: center;    color: #fff;        font-size: 14px;    display: block;    border-radius: 4px;">Add to Cart </span>
                </a>';
        $notification['message'] .= '   
      <tr>
	    ...
	    <td style="padding:12px;border: 1px solid #e2ddd3;">'                        . '$' . $prod11Price . '</td>        
	    <td style="padding:12px;border: 1px solid #e2ddd3; text-align:center;">'     . $addToCart  . '</td>
      </tr>';
     }   
...
//product 12   
 if ($product12=='true'){
	 	 	 $addToCart = '<a target="_blank" rel="noopener" href="https://rubnrestore.com/cart/?wpo_fill_cart=' .  $product12addtoCART . '" > ' . $product12IMG . ' <span style="    padding: 5px;    background: #016ab5;    text-align: center;    color: #fff;        font-size: 14px;    display: block;    border-radius: 4px;">Add to Cart </span>
                </a>';
        $notification['message'] .= '   
      <tr>
	    ...
	    <td style="padding:12px;border: 1px solid #e2ddd3;">'                        . '$' . $prod12Price . '</td>        
		<td style="padding:12px;border: 1px solid #e2ddd3; text-align:center;">'     . $addToCart  . '</td>
      </tr>';
     }    
...
//product 13   
 if ($product13=='true'){
	 	 	 ...
	 	 	 $addToCart = '<a target="_blank" rel="noopener" href="https://rubnrestore.com/cart/?wpo_fill_cart=' .  $product13addtoCART . '" > ' . $product13IMG . ' <span style="    padding: 5px;    background: #016ab5;    text-align: center;    color: #fff;        font-size: 14px;    display: block;    border-radius: 4px;">Add to Cart </span>
                </a>';
        $notification['message'] .= '   
      <tr>
	    ...
	    <td style="padding:12px;border: 1px solid #e2ddd3;">'                        . '$' . $prod13Price . '</td>     
		<td style="padding:12px;border: 1px solid #e2ddd3; text-align:center;">'     . $addToCart . '</td>
      </tr>';
     }  	 
...
//product 14  
 if ($product14=='true'){
	 	 	 ...
	 	 	 $addToCart = '<a target="_blank" rel="noopener" href="https://rubnrestore.com/cart/?wpo_fill_cart=' .  $product14addtoCART . '" > ' . $product14IMG . ' <span style="    padding: 5px;    background: #016ab5;    text-align: center;    color: #fff;        font-size: 14px;    display: block;    border-radius: 4px;">Add to Cart </span>
                </a>';
        $notification['message'] .= '   
      <tr>
	    ...
	    <td style="padding:12px;border: 1px solid #e2ddd3;">'                        . '$' . $prod14Price . '</td>        
	    <td style="padding:12px;border: 1px solid #e2ddd3; text-align:center;">'     . $addToCart . '</td>
      </tr>';
     }  
...
//addto cart row  
//Remove add all to cart. Updated to individual add to cart buttons and generic color 6-23-2023 AJ Designs
...
/* $notification['message'] .= '  
    <tr>
        <th  scope="row" colspan="1" style="vertical-align:middle; padding:30px; text-align:left;border: 1px solid #e2ddd3; font-style:bold;"> </th>
             
        <td  colspan="4" style="vertical-align:middle;padding:30px;text-align:left;border:1px solid #e2ddd3;">
                
                <a target="_blank" rel="noopener" href="https://rubnrestore.com/cart/?wpo_fill_cart=' . $productCM0addtoCART . $product1addtoCART .  $product2addtoCART . $product3addtoCART . $product4addtoCART . $product4_1addtoCART . $product4_12addtoCART . $product5addtoCART . $product5_01addtoCART . $product6addtoCART . $product6_1addtoCART . $product7addtoCART . $product8addtoCART . $product9addtoCART . $product10addtoCART . $product11addtoCART . $product12addtoCART . $product13addtoCART . $product14addtoCART .'" style="    padding: 10px;    background: #016ab5;    text-align: center;    color: #fff;    font-weight: bold;    font-size: 16px;    display: block;    border-radius: 4px;">Add All to Cart
                </a>
...
<!--Repair-->
<table width="600" border="1" cellspacing="0" cellpadding="0" style="
gin-bottom:40px; background-color:#fff; font-size:15px;">
  <tr>
